    ...
    constructor() {
        ...
        this.upgrade = () => {
            ...
            const main$ = responseOk$.pipe(map(context => this.updateManager.responseToContent(context)), tap(context => this.eventManager.emitReady(context)), observeOn(animationFrame), tap(context => {
                ...
                url: this._url,
                scripts: [],
            }), observeOn(animationFrame), tap(context => this.scrollManager.manageScrollPosition(context)), tap({ error: e => this.eventManager.emitDOMError(e) }), catchError((_, c) => c), switchMap((x) => this.fadePromise.then(() => x)), switchMap(x => this.updateManager.reinsertScriptTags(x)), tap({ error: e => this.eventManager.emitError(e) }), catchError((_, c) => c), tap(context => this.eventManager.emitLoad(context)));
