...
function isHashChange({ cause, url: { pathname, hash }, oldURL: { pathname: prevPathname }, }) {
    ...
    setupEventListeners() {
        ...
        const hintEvent$ = this.$.linkSelector.pipe(switchMap((linkSelector) => {
            ...
            return createMutationObservable(this.el, { childList: true, subtree: true }).pipe(startWith({ addedNodes: [this.el], removedNodes: [] }), bufferDebounceTime(500), map(combineRecords), switchMap(({ addedNodes, removedNodes }) => {
                from(removedNodes).pipe(filter(el => el instanceof Element), mergeMap((el) => matchOrQuery(el, linkSelector)), tap(removeLink)).subscribe();
                from(addedNodes).pipe(filter(el => el instanceof Element), mergeMap((el) => matchOrQuery(el, linkSelector)), tap(addLink)).subscribe();
...
let HyPushState = class HyPushState extends applyMixins(RxLitElement, [EventListenersMixin]) {
    constructor() {
        ...
        this.upgrade = () => {
            ...
            const hash$ = merged$.pipe(filter(p => isHashChange(p)), filter(() => history.state && history.state[this.histId]), tap(context => {
            ...
            const main$ = responseOk$.pipe(map(context => this.updateManager.responseToContent(context)), tap(context => this.eventManager.emitReady(context)), observeOn(animationFrame), tap(context => {
                ...
                url: this._url,
                scripts: [],
            }), tap(context => this.scrollManager.manageScrollPosition(context)), tap({ error: e => this.eventManager.emitDOMError(e) }), catchError((_, c) => c), switchMap(x => this.updateManager.reinsertScriptTags(x)), tap({ error: e => this.eventManager.emitError(e) }), catchError((_, c) => c), tap(context => this.eventManager.emitLoad(context)));
